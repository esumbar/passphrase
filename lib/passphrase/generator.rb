...
module Passphrase
	class Generator
		...
		def run
			...
			list_selector = Random.new(num_words, 0, word_list.length - 1, @options.local).to_array
			num_words.times do |iword|
				word_hash = Random.new(5, 1, 6, @options.local).to_array.join.to_sym
		...
		def one_random_number(max)
			Random.new(1, 0, max, @options.local).to_array.shift
		end
	end
end
